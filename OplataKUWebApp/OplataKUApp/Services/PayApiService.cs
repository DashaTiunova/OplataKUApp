...
namespace OplataKUWebApp.Services
{
    public class PayApiService
    {
        ...
        public PayApiService(IHttpClientFactory clientFactory)
        {
            _httpClient = clientFactory.CreateClient("ClientApi");
        }

        //private string _host = "http://localhost:5012";
        public async Task<List<PayDto>?> GetPayInfo(PayFilterDto filter)
        {

            var response = await _httpClient.PostAsJsonAsync("/PayInfo/GetAll", filter);
